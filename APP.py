...
def load_questions_answers(file):
    try:
        ...
        # --- REMOVED HEADER SKIP LOGIC HERE (next(reader)) ---
...
def save_results_to_csv(results_data, student_name, student_roll_no):
    ...
    metadata_rows = pd.DataFrame([
        {'Question': 'Student Name', 'Correct Answer (Expected)': student_name, 'Student Answer (Raw)': '', 'Score': '', 'Feedback': ''},
        {'Question': 'Roll Number', 'Correct Answer (Expected)': student_roll_no, 'Student Answer (Raw)': '', 'Score': '', 'Feedback': ''},
        {'Question': '--- Assessment Details ---', 'Correct Answer (Expected)': '', 'Student Answer (Raw)': '', 'Score': '', 'Feedback': ''}
    ])
    ...
    df = pd.concat([metadata_rows, df, summary_rows], ignore_index=True)
...
def main():
    ...
    st.info("Your CSV should have at least two columns: `Question` (Column 1) and `Answer` (Column 2). **The first row of your CSV will now be treated as a question.**")
    ...
    if questions_file:
        ...
        if questions_answers:
            st.subheader(f"2. Input Student Information and Answers ({len(questions_answers)} Questions Found)")
            
            col_info_1, col_info_2 = st.columns(2)
            student_name = col_info_1.text_input("Student Name", key="student_name_input", placeholder="e.g., Jane Doe")
            student_roll_no = col_info_2.text_input("Roll Number", key="student_roll_no_input", placeholder="e.g., 20241001")
            st.markdown("---")
            ...
            if st.button("3. 🚀 Grade and Generate Feedback", use_container_width=True, type="primary"):
                if not student_name or not student_roll_no:
                    st.error("Please enter the Student Name and Roll Number before grading.")
                    return
                ...
                csv_bytes = save_results_to_csv(results_data, student_name, student_roll_no)
                st.download_button(
                    "Download Full Feedback Report (CSV)", 
                    csv_bytes, 
                    file_name=f"Assessment_Report_{student_roll_no}.csv", 
                    mime="text/csv",
                    use_container_width=True
                )
